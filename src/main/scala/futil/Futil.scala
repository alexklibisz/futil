...
import java.util.concurrent.atomic.{AtomicInteger, AtomicIntegerArray, AtomicReferenceArray}
...
trait Futil {
  ...
  /**
    ...
    */
  final def deadline[A](duration: Duration)(fa: => Future[A])(implicit ec: ExecutionContext,
  ...
  /**
    ...
    */
  final def mapParN[A, B](n: Int)(as: IndexedSeq[A])(f: A => Future[B])(
      implicit ec: ExecutionContext): Future[IndexedSeq[Try[B]]] =
  ...
  /**
    * Alias for [[mapParN]] with n = 1. Executes the
    */
  final def mapSerial[A, B](as: IndexedSeq[A])(f: A => Future[B])(
      implicit ec: ExecutionContext): Future[IndexedSeq[Try[B]]] =
    mapParN(1)(as)(f)
